...
UIS.InputBegan:Connect(function(Input, Processed)
	...
	if Input.KeyCode == Enum.KeyCode.Semicolon then
		...
		Anims = {
			TS:Create(CommandBar, TweenInfo.new(.4, Enum.EasingStyle.Cubic), {BackgroundTransparency = .25, Position = UDim2.new(.233,0,.877,0)}),
			TS:Create(CommandBar.Icon, TweenInfo.new(.4, Enum.EasingStyle.Cubic), {ImageTransparency = 0}),
			TS:Create(CommandBar.Label, TweenInfo.new(.4, Enum.EasingStyle.Cubic), {TextTransparency = 0}),
		}
		for i, Anim in Anims do Anim:Play() end
		...
		local SearchEvent = CommandBar.Label:GetPropertyChangedSignal("Text"):Connect(function()
			task.spawn(function() --// spawn so we can return IfAnimating safely
				...
				ResultsBox.Position = UDim2.new(.233,0,.877,0)
				ResultsBox.Size = UDim2.new(.534,0,.092,0)
				
				Anims = {
					TS:Create(CommandBar, TweenInfo.new(.4, Enum.EasingStyle.Cubic), {BackgroundTransparency = 1}),
					TS:Create(ResultsBox, TweenInfo.new(.4, Enum.EasingStyle.Cubic), {GroupTransparency = 0, Size = UDim2.new(.534,0,.318,0), Position = UDim2.new(.233,0,.648,0)}),
				}
				for i, Anim in Anims do Anim:Play() end
			end)
			...
			Anims = { --// child is 2nd here bc of the UIGridLayout
				TS:Create(CommandCursor, TweenInfo.new(.4, Enum.EasingStyle.Cubic), {Position = ResultsBox.ScrollingFrame:GetChildren()[2].RunSymbol.Position}),
				TS:Create(ResultsBox.ScrollingFrame:GetChildren()[2].AppIcon, TweenInfo.new(.4, Enum.EasingStyle.Cubic), {Position = ResultsBox.TemplateWithArrow.AppIcon.Position}),
			}
			for i, Anim in Anims do Anim:Play() end
		end)
